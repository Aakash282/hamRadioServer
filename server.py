...
class TCPHandler(SocketServer.BaseRequestHandler):
    ...
    The handler will dispatch 
    """

    def handle(self):
        # self.request is the TCP socket connected to the client
        self.data = self.request.recv(1024).strip()
        data = self.data.split()[1][1:]
        # data = data.strip('%5B').strip('%5D')
        currentPlaylist = data.strip('/').strip('[').strip(']').split(',')
        ...
        self.request.sendall(str(tracksToAdd))
...
# initiates server
# loads in local database
# calls request handler
if __name__ == "__main__":
    HOST, PORT = "", 8080

    # load in the locally stored attributes
    loadEchonestAttributes()

    # Create the server, binding to our public IP on port 8080
